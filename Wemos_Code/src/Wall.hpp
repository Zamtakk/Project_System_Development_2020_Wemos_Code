...
void handleMessage(JsonObject message)
{
    ...
    case DEVICEINFO:
    {
        ...
        deviceInfoMessage["lightSensorValue"] = analogInput0Value;
        deviceInfoMessage["potmeterValue"] = analogInput1Value;
        deviceInfoMessage["curtainsState"] = output0State;
...
void handleAnalogInput()
{
    ...
    if (abs(analog0In_avg - analog0In_Previous) > 20)
    {
        ...
        sendIntMessage(WALL_LDR_CHANGE, analogInput0Value);
    }
    if (abs(analog1In_avg - analog1In_Previous) > 20)
    {
        analog1In_Previous = analog1In_avg;
        analogInput1Value = analog1In_avg / 1023.0 * 255.0;
        Serial.printf("Forcesensor value: %d\n", analogInput1Value);
        sendIntMessage(WALL_POTMETER_CHANGE, analogInput1Value);
    }
}
